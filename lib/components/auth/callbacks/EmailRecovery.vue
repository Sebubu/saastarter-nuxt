<template>
  <div>
    ...
    <div v-if="!checkingActionCode && !isActionCodeValid">
      <p class="text-center text-3xl">Email recovery failed.</p>
    ...
    <div v-if="!checkingActionCode && isActionCodeValid">
      <div class>
        <p class="text-center text-3xl">Success. Email recovery.</p>
        <p
          class="text-center text-md pt-3 md:pt-8"
        >Your email address has been restored to {{restoredEmail}}.</p>
        <nuxt-link to="/">
          <button
            type="submit"
            class="saastarter-btn-secondary w-full text-lg font-bold p-2 mt-8"
...
export default Vue.extend({
  ...
  mounted: async function () {
    try {
      const info = await this.$fireAuth.checkActionCode(this.actionCode);
